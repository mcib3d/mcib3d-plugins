...
public class Thresholder {
     ...
     *
     * @param image the input image to be thresholded. The image is overwritten with the thresholding results.
     * @param low   {@code high} - the lower and higher threshold values that define the hysteresis. Elements with a value larger than or equal to {@code high} are set to {@code 255},
     *              elements with a value less than {@code low} are set to {@code 0}, and elements with a value larger than or equal to the {@code low} are set to {@code 255}
     *              only if they are connected to elements with a value larger than or equal to {@code high} through elements all with a value larger than or equal to {@code low}.
     *              If the size of the image in the z-dimension equals {@code 1}, this method uses 8-connectivity in x-y space, otherwise it uses 26-connectivity in x-y-z space.
     *              The algorithm is applied to every x-y(-z) subimage in a 5D image.
    ...
    public void hysteresis(final Image image, final double low, final double high) {
        ...
        if (dims.z == 1) { // 2D case
            ...
            final ByteImage map = new ByteImage(mdims);
            map.axes(Axes.X);
    ...
    public void hysteresisLowConnectivity(final Image image, final double low, final double high) {
        ...
        if (dims.z == 1) { // 2D case
                ...
                for (mcoords.t = 0; mcoords.t < dims.t; ++mcoords.t) {
                    for (mcoords.y = 1; mcoords.y <= dims.y; ++mcoords.y) {
                        ...
                        for (mcoords.x = 1; mcoords.x <= dims.x; ++mcoords.x) {
                            if (marray[mcoords.x] == ON) {
                                cands.push(mcoords);
                                // Search for candidates iteratively in 4-connected neighborhoods:
                                while (cands.size > 0) {
                                    cands.pop(cand);
                                    map.set(cand, ON);
                                    ++cand.x;
            ...
            // TODO ne pas creer une nouvelle map, utiliser la map actuelle..ou copier la bouding box..
            // Create map with border:
            final Dimensions mdims = new Dimensions(dims.x + 2, dims.y + 2, dims.z + 2, dims.t, dims.c);
            final ByteImage map = new ByteImage(mdims);
            map.axes(Axes.X);
                    ...
                    for (mcoords.z = 1; mcoords.z <= dims.z; ++mcoords.z) {
                        for (mcoords.y = 1; mcoords.y <= dims.y; ++mcoords.y) {
                            ...
                            for (mcoords.x = 1; mcoords.x <= dims.x; ++mcoords.x)
                                if (marray[mcoords.x] == ON) {
                                    cands.push(mcoords);
                                    // Search for candidates iteratively in 6-connected neighborhoods:
                                    while (cands.size > 0) {
                                        cands.pop(cand);
                                        map.set(cand, ON);
                                        ++cand.z;
