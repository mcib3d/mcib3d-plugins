...
int createKernelEllipsoid2(jbyte* ker, int vx, int vy, int vz);
   ...
   // unsigned short* tmpmin2;
    ...
    for (i = 0; i < p; i++) {
        ...
        for (j = 0; j < s; j++) {
            ...
            idx = 0;
            for (z = -rz; z <= rz; z++) {
                if ((i+z <0) || (i+z >= p)) continue;
                for (y = -ry; y <= ry; y++) {
                    for (x = -rx; x <= rx; x++) {
                        if ( (j + x + w * y >= 0) && (j + x + w * y < s)) {
                            //dist = (x * x) / rx2 + (y * y) / ry2 + (z * z) / rz2;
                            if (ker[idx]) {
...
(JNIEnv * env, jobject obj, jobjectArray tab, jint w, jint h, jint p, jint rx, jint ry, jint rz) {
   ...
   // unsigned short* tmpmin2;
    ...
    jbyte* ker = (jbyte *) malloc((2 * rx + 1) * (2 * ry + 1) * (2 * rz + 1) * sizeof (jbyte));
    //printf("creating kernel\n");
    c2 = createKernelEllipsoid2(ker, rx, ry, rz);
    //printf("creating kernel done\n");
    //for(j=0;j<(2*rx+1)*(2*ry+1)*(2*rz+1);j++) printf("test %d %d \n",j,ker[j]);
    int idx;
                    ...
                    for (x = -rx; x <= rx; x++) {
                     {
                            //dist = (x * x) / rx2 + (y * y) / ry2 + (z * z) / rz2;
                                ...
                                sum += (ker[idx]*element[i + z][j + x + w * y]&0xffff);
